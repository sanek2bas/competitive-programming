...
public class CopyListWithRandomPointer
{
    ...
    /// to new nodes in the copied list such that the pointers in 
    ...
    /// of n nodes. Each node is represented as 
    ...
    /// </summary>
    public ListNodeWithRandom Execute(ListNodeWithRandom head)
    {
        return Copy(head, new Dictionary<ListNodeWithRandom, ListNodeWithRandom>());
    }

    private ListNodeWithRandom Copy(ListNodeWithRandom head, Dictionary<ListNodeWithRandom, ListNodeWithRandom> dic)
    {
        if (head == null)
            return null;
